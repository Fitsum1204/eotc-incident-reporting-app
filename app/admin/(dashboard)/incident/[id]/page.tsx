...
export default async function IncidentDetail({params}: {params: Promise<{id:string}>}) {
  ...
  const { data: posts } = await sanityFetch({ query: INCIDENT_QUERY  ,params:{id} });
  if(!posts) return notFound();
    ...
    <>
      <div className="max-w-3xl mx-auto px-4 py-10">
           ...
           <div className="relative w-full h-64 sm:h-80 md:h-96 overflow-hidden rounded-xl my-2">
            <OneIncidentMapClient incident={posts} /> </div>
            {/* Metadata */}
      ...
      {/* Related Incidents */}
      {posts.relatedIncidents && posts.relatedIncidents.length > 0 && (<h1>Related incidents</h1>)}
